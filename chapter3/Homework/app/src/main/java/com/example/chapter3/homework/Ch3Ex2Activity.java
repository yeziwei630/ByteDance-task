...
public class Ch3Ex2Activity extends AppCompatActivity {
    ...
    private void resetTargetAnimation() {
        ...
        animatorSet.playTogether(colorAnimator, scaleXAnimator, scaleYAnimator, alphaAnimator);

        // 设置animatorSet子动画的duration, repeatCount, repeatMode
        // 避免出现重复代码
        ArrayList<Animator> animatorList = animatorSet.getChildAnimations();
        for (Animator animator : animatorList) {
            ObjectAnimator objectAnimator = (ObjectAnimator) animator;
            objectAnimator.setDuration(Integer.parseInt(durationSelector.getText().toString()));
